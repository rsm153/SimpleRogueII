...
public class SimpleRogueII extends Application {
  ...
  public void spawnStairs(){
     ...
     * where the stairs go.
     * The trick to this is not calculating for the "last" x and y, and if the x or y matches the player's, 
     * put it on the last one instead
     * 
     * ie.
     * #..@..#
     * random 0-4
     * 0->1, 1->2, 2->5, 3->4
     * 3 aren't calculated for, so the x and y is [0, max - 3] (or [1, max - 2]
     */
    int stairsX = randIntBetween(1, sideLengthX - 2);
    int stairsY = randIntBetween(1, sideLengthY - 2);
    stairsX = (stairsX == playerX) ? sideLengthX - 1 : stairsX;
    stairsY = (stairsY == playerY) ? sideLengthY - 1 : stairsY;
    setSpriteOnMap(sprite('='), stairsX, stairsY);
  }
  
  public void nextLevel(int levelIncrease){
    /*
     * The player has reached the stairs, so they can proceed to the next level:
     *   Increment the current level by levelIncrease (usually 1)
     *   Clear the floor
     *   Respawn player where they were
     *   Respawn the stairs in new location
     *   Spawn enemies and items relative to the power level
     */
    spawnPlayer();
    spawnStairs();
  }
  ...
  public void movePlayer(int x, int y){
    ...
    if(spriteB == sprite('.')){ //if the move is valid == if the tile is floor
      ...
      playerY -= y;                                              
    } else if (spriteB == sprite('=')){ //if move onto stairs, entire floor should change, for now just respawn stairs.
      setSpriteOnMap(sprite('.'), playerX, playerY);             // set label player was on to a floor sprite
      setSpriteOnMap(sprite('@'), playerX + x, playerY - y);     // set label player is now on to player sprite
      playerX += x;                                              // set player location to new location
      playerY -= y;
